...
class ShowExchange extends Component
{
    ...
    public function mount()
    {
        ...
        $this->ordersClose = Order::where('person_id', $this->person->id)
            ->where('status','closed')                    
            ->get();
        $this->ordersHistory = Order::where('person_id', $this->person->id)->where('executed', false)->get();
    }

    public function render()
    {
        return view('livewire.show-exchange')->layout('layouts.exchange');
    }
    ...
    public function orderScoreBuy()
    {
       ...
       //dd($askPrice, $this->amountbuy, ($this->amountbuy / $askPrice), $wallet->amount + ($this->amountbuy / $askPrice));
        ...
        $wallet->update([
            ...
            'amount' => $wallet->amount + ($this->amountbuy / $askPrice),
        ]);
    ...
    public function orderScoreSell()
    {
        ...
        if ($wallet->amount <= $this->amountsell) {
        ...
        $wallet->update([
            'amount' => $wallet->amount - $amountsellNumerico,
